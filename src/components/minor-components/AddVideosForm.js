...
import ImageHolder from '../../assets/video.svg';
...
export const AddVideosForm = (props) => {
                    ...
                    <div className="flex justify-center">
                        <div className="w-full flex h-screen">
                            ...
                            <div className="w-full xl:w-[65%] md:w-full bg-white rounded-lg ">
                                <h3 className="pt-4 text-2xl text-center mt-8 font-bold">{!status ? 'Video Details' : "Add Video"}</h3>
                                <Formik
                                    enableReinitialize
                                    initialValues={
                                        {
                                            ...
                                            title: editItem.length !== 0 ? `${editItem[0].title}` : '',
                                        } 
                                    }
                                    ...
                                    onSubmit={async (values) => {
                                        ...
                                        if (!status) {
                                            ...
                                            dispatch(addVideo(values, formData, navigate, alert, props.modal, setUploadProgress))
                                        }
                                    }}
                                >
                                            ...
                                            <div className="flex mx-auto justify-center">
                                                <div className=" md:mr-2 md:mb-0 md:w-full justify-center mx-auto">
                                                    <label htmlFor="upload" className='w-100 h-100 block rounded-[50%] cursor-pointer mx-auto mb-2'>
                                                        {/* {!filePreview ? 
                                                            <img className='w-[125px] h-[125px] block rounded-[50%] cursor-pointer mb-2 ' src={!status && editItem.length !== 0 && !imgCheck ? baseURL + editItem[0].categoryImage : !values.video ? ImageHolder : filePreview} alt='img' />
                                                            : null} */}
                                                        {values.video  ?
                                                        ...
                                                        <input className='hidden' id="upload" name="image" type="file" accept="image/png, image/jpeg, video/mp4, video/x-m4v, video/*, application/pdf" onChange={(event) => {
                                                    ...
                                                    <label className="block mb-2 text-sm font-bold text-gray-700 md:mt-2 text-center " name='video'>
                                                        Add Multimedia
                                                    </label>
                                                    <ErrorMessage className='text-red-600 text-xs text-center' name="video" component="div" />
                                                </div>
                                            </div>
                                            {uploadProgress > 0 && (
                                                    ...
                                                    <div className='flex flex-col justify-around my-3'>
                                                        <div className=" md:flex md:justify-between w-60 xl:w-44 lg:w-36 md:w-full md:mb-0">
                                                            <div className=" md:mr-2 md:mb-0 md:w-full">
                                                                ...
                                                                <Field className='w-full px-3 py-2  text-xs leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline' type="text" name="title" />
                                                                <ErrorMessage className='text-red-600 text-xs font-thin' name="title" component="div" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
