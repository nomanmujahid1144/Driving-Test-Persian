...
import { addQuestions, getEngCategoryQuestions, updateQuestion } from '../../redux/Actions/TestQuestionsAction';
...
export const AddTestQuestionForm = (props) => {
    ...
    useEffect(() => {
        if (!status) {
            console.log(engQuestions, 'engQuestions')
            const editQuestion = engQuestions?.questions?.filter(
                (product , index) => index ===  global.testEditId
            )
                    ...
                    <div className="flex justify-center">
                        <div className="w-full flex ">
                            ...
                            <div className="w-full xl:w-[65%] md:w-full bg-white rounded-lg ">
                                <h3 className="pt-4 text-2xl text-center mt-8 font-bold">{!status ? 'Test Question Details' : "Add Test Question"}</h3>
                                <Formik
                                    enableReinitialize
                                    initialValues={
                                        {
                                    ...
                                    onSubmit={async (values) => {
                                        ...
                                        if (!status) {
                                            if (imgCheck) {
                                                ...
                                                dispatch(updateQuestion(values, formData, navigate, alert, props.modal , params.id, editId))
                                            }
                                            else {
                                                dispatch(updateQuestion(values, formData, navigate, alert, props.modal , params.id , editId))
                                            }
